...
describe.only('FluxAggregator', () => {
  ...
  const deployment = setup.snapshot(provider, async () => {
    ...
    log.debug(`aggregator deploy cost was: ${receipt.gasUsed} gas`)
    log.debug(`aggregator deployed`)
    ...
    describe('adding oracles', () => {
      ...
      describe(`when adding more than ${limit} oracles`, () => {
        ...
        it.only('not use too much gas', async () => {
          ...
          if (tx) {
            const receipt = await tx.wait()
            log.debug(`"not use too much gas" flow used ${receipt.gasUsed.toNumber()}`)
